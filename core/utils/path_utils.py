...
def get_user_data_directory():
    ...
    %LOCALAPPDATA%/ArchiveSift/UserData 폴더(Windows) 또는
    ~/ArchiveSift/UserData 폴더(macOS, Linux)에 저장합니다.
    ...
    if os.name == 'nt':  # Windows
        # %LOCALAPPDATA%/ArchiveSift 경로 사용 (권한 문제 방지)
        local_app_data = os.environ.get('LOCALAPPDATA')
        if not local_app_data:  # 환경 변수가 없는 경우 대체 경로
            local_app_data = os.path.join(os.path.expanduser('~'), 'AppData', 'Local')
        data_dir = os.path.join(local_app_data, 'ArchiveSift', 'UserData')
    else:  # macOS, Linux 등
