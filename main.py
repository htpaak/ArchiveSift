...
from core.initializer import MediaSorterPAAKInitializer
...
# 메인 이미지 뷰어 클래스 정의
class MediaSorterPAAK(QWidget):
    def __init__(self):
        super().__init__()  # 부모 클래스 초기화        
        # 초기화 위임
        initializer = MediaSorterPAAKInitializer()
    ...
    def resizeEvent(self, event):
        ...
        # --- 추가: 창 크기 변경 시 폴더 버튼 업데이트 ---
        if hasattr(self, 'update_folder_buttons'):
             print("DEBUG: resizeEvent calling update_folder_buttons") # 디버그 로그 추가
    ...
    def set_base_folder(self):
        ...
        if folder_path:  # When folder is selected
            ...
            # # 하위 폴더 목록 가져오기 -> update_folder_buttons에서 처리
            ...
            # # 버튼 너비 계산 -> update_folder_buttons에서 처리
            # # button_width = self.width() // 20

            # # 폴더 버튼 업데이트 루프 -> update_folder_buttons에서 처리
            # for i, row in enumerate(self.buttons):
            #     for j, button in enumerate(row):
            #         # ... (기존 로직 주석 처리 또는 삭제) ...
            # --- 중복 코드 제거 끝 ---
            
            # # Undo 버튼 텍스트 다시 설정 -> update_folder_buttons에서 처리
            # if hasattr(self, 'undo_button') and self.undo_button:
            #     self.undo_button.setText("Undo")
            
            # --- 추가: update_folder_buttons 호출 ---
            self.update_folder_buttons()
            # --- 추가 끝 ---
    ...
    def update_window_title(self, image_path):
        """창 제목과 제목표시줄 업데이트"""
        # 파일 이름을 제목표시줄에 표시
        file_name = os.path.basename(image_path) if image_path else "MediaSorterPAAK"
        title_text = f"MediaSorterPAAK - {file_name}" if image_path else "MediaSorterPAAK"
    ...
    def open_feedback(self):
        ...
        feedback_url = "https://github.com/htpaak/MediaSorterPAAK/discussions"
        ...
        for i, row in enumerate(self.buttons):
            for j, button in enumerate(row):
                ...
                if button == self.undo_button:
                    # Undo 버튼 폰트 크기 초기화 (다른 버튼과 다를 수 있으므로)
                    if hasattr(button, 'setFont'):
...
# Main function
def main():
    # --- 추가: High DPI 스케일링 활성화 ---
    # QApplication 인스턴스 생성 전에 호출해야 함
    QApplication.setAttribute(Qt.AA_EnableHighDpiScaling, True)
    # --- 추가 끝 ---
    
    app = QApplication(sys.argv)  # Qt application instance creation
    app.setApplicationName("MediaSorterPAAK")  # Set application name
    
    viewer = MediaSorterPAAK()  # Create instance of MediaSorterPAAK class
